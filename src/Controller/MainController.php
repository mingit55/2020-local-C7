...
class MainController {
    ...
    function partyPage(){
        $sql = "SELECT DISTINCT K.*, user_name, user_id, IF(R.cnt IS NULL, '0', floor(R.total / R.cnt)) score, M.kid reviewed
                FROM knowhows K
                INNER JOIN users U ON U.id = K.uid
    ...
    function writeKnowhow(){
        ...
        move_uploaded_file($before_img['tmp_name'], _UPLOAD."/knowhows/$before_name");
        move_uploaded_file($after_img['tmp_name'], _UPLOAD."/knowhows/$after_name");

        DB::query("INSERT INTO knowhows(before_img, after_img, contents, uid) VALUES (?, ?, ?, ?)", [$before_name, $after_name, $contents, user()->id]);
        go("/online-party", "글이 작성되었습니다.");
    }
    function reviewKnowhow(){
        extract($_POST);
        DB::query("INSERT INTO knowhow_reviews(uid, kid, score) VALUES (?, ?, ?)", [user()->id, $kid, $score]);
        
        $data = DB::fetch("SELECT COUNT(*) cnt, SUM(score) total FROM knowhow_reviews WHERE kid = ? ", [$kid]);
        $score = !$data->cnt ? 0 : (int)($data->total / $data->cnt);
    ...
    function estimatePage(){
        $sql = "SELECT Q.*, user_name, user_id, ifnull(S.cnt, 0) cnt, M.qid sended
                FROM requests Q
        ...
        $sql = "SELECT S.*, user_id, user_name, sid, contents, start_date
                FROM responses S
    ...
    function getResponses(){
        ...
        $list = DB::fetchAll("SELECT S.*, user_name, user_id FROM responses S, users U WHERE S.uid = U.id AND S.qid = ?", [$req->id]);
        json_response(["request" => $req, "list" => $list]);
    }
